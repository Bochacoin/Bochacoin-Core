...
CzBOCHAWallet::CzBOCHAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbocha seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBOCHASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBOCHASeed_deprecated()) {
                    LogPrintf("%s: Updated zBOCHA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbocha seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbocha wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBOCHAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBOCHACount(nCountLastUsed);
    else if (!walletdb.ReadZBOCHACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBOCHAWallet::Lock()
{
    seedMaster = 0;
}

void CzBOCHAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBOCHAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBOCHA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBOCHAWallet::LoadMintPoolFromDB()
{
...
void CzBOCHAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBOCHAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBOCHAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbochaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBOCHAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBOCHA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbochaTracker which also adds to database
    pwalletMain->zbochaTracker->Add(dMint, true);
...
void CzBOCHAWallet::SeedToZBOCHA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBOCHAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBOCHAWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBOCHACount(nCountLastUsed);
...
void CzBOCHAWallet::GenerateDeterministicZBOCHA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBOCHAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBOCHAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
