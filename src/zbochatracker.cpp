...
CzBOCHATracker::CzBOCHATracker(std::string strWalletFile)
{
...
CzBOCHATracker::~CzBOCHATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBOCHATracker::Init()
{
...
bool CzBOCHATracker::Archive(CMintMeta& meta)
{
...
bool CzBOCHATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBOCHATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBOCHATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBOCHATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBOCHATracker::GetSerialHashes()
{
...
CAmount CzBOCHATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBOCHATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBOCHATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBOCHATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBOCHATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBOCHATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBOCHATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBOCHATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBOCHATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBOCHATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBOCHATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBOCHATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBOCHATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBOCHATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBOCHATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBOCHATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBOCHATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbocha from DB\n", __func__, listDeterministicDB.size());
    }
