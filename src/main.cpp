...
#if defined(NDEBUG)
#error "Bochacoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubocha) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zBOCHA is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zBOCHA spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zBOCHA without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zBOCHA spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zBOCHA spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBOCHA spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbocha");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zBOCHA mints are not already known
 	...
 	if (nValueOut > 9998.99 * COIN && nValueOut < 10002 * COIN) {
       return false;	}
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
     nSubsidy = 5050000000 * COIN;
    }
    if (nHeight <= 100000 && nHeight > 0) {
     nSubsidy = 10 * COIN;
    }
    if (nHeight <= 300000 && nHeight > 100000) {
     nSubsidy = 9.5 * COIN;
    }
    if (nHeight <= 500000 && nHeight > 300000) {
     nSubsidy = 9.02 * COIN;
    }
    if (nHeight <= 1000000 && nHeight > 500000) {
     nSubsidy = 8.56 * COIN;
    }
    if (nHeight <= 1500000 && nHeight > 1000000) {
     nSubsidy = 8.13 * COIN;
    }
    if (nHeight <= 2500000 && nHeight > 1500000) {
     nSubsidy = 7.72 * COIN;
    }
    if (nHeight <= 3800000 && nHeight > 2500000) {
     nSubsidy = 7.33 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBOCHAStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Bochacoin
...
{
    RenameThread("bochacoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBOCHAMinted()
{
...
void RecalculateZBOCHASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBOCHA supply
...
bool RecalculateBOCHASupply(int nHeightStart)
{
...
bool UpdateZBOCHASupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateBOCHASupply(Params().Zerocoin_StartHeight());
    }

    //Track zBOCHA money supply in the block index
    if (!UpdateZBOCHASupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zBOCHA supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BOCHA to zBOCHA
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBOCHA spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBOCHA serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zBOCHA stake block is less than allowed start height", __func__);

    if (CZPivStake* zBOCHA = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zBOCHA->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zBOCHA stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zBOCHA stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zBOCHA->GetValue()));
        if (nChecksum200 != zBOCHA->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zBOCHA->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZBOCHA() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zBOCHA fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBOCHA mints and %d zBOCHA spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Bochacoin: We use certain sporks during IBD, so check to see if they are
