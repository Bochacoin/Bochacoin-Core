...
// The zBOCHA block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBOCHA is a hash of the serial
...
{
    //Create an output returning the zBOCHA that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBOCHAOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBOCHA output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBOCHA", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBOCHAOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBOCHA output", __func__);
...
{
    CzBOCHATracker* zbochaTracker = pwallet->zbochaTracker.get();
    CMintMeta meta;
    if (!zbochaTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbochaTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BOCHA stake is the outpoint
