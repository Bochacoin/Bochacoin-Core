...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BOCHA);
    ...
    if (!settings.contains("nAnonymizeBochacoinAmount"))
        settings.setValue("nAnonymizeBochacoinAmount", 1000);

    nAnonymizeBochacoinAmount = settings.value("nAnonymizeBochacoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBochacoinAmount"))
        SoftSetArg("-anonymizebochacoinamount", settings.value("nAnonymizeBochacoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bochacoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBochacoinAmount:
            return QVariant(nAnonymizeBochacoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBochacoinAmount:
            nAnonymizeBochacoinAmount = value.toInt();
            settings.setValue("nAnonymizeBochacoinAmount", nAnonymizeBochacoinAmount);
            emit anonymizeBochacoinAmountChanged(nAnonymizeBochacoinAmount);
            break;
        case CoinControlFeatures:
