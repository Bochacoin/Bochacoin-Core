...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bochacoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BOCHA.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bochacoin:URI (bochacoin:XyZ...?message=example)
            {
...
static void NotifyzBOCHAReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBOCHAReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBOCHAReset.connect(boost::bind(NotifyzBOCHAReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBOCHAReset.disconnect(boost::bind(NotifyzBOCHAReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbochaTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
